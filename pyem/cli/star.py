...
def main(args):
    ...
    if args.split_micrographs:
        ...
        for mg in dfs:
            if not args.relion2:
                df = star.remove_deprecated_relion2(dfs[mg])
                star.write_starfile(os.path.join(args.output, os.path.basename(mg)[:-4]) + args.suffix, df,
                                    resort_records=args.sort, simplify=args.augment_output, optics=True)
            else:
                df = star.remove_new_relion31(dfs[mg])
                star.write_starfile(os.path.join(args.output, os.path.basename(mg)[:-4]) + args.suffix, df,
                                    resort_records=args.sort, simplify=args.augment_output, optics=False)
        return 0
